<x-app-layout>
    ...
    <div class="bg-white rounded-xl border border-gray-100 overflow-hidden divide-y divide-gray-50 shadow-sm">
        @foreach($categories as $cat)
            @php
                ...
                $icon = $staticCat['icon'] ?? 'dots-horizontal';
                $color = $staticCat['color'] ?? 'blue';
            @endphp
            <div class="p-4 flex items-center justify-between transition-all active:bg-gray-50">
                <div class="flex items-center gap-4 text-left">
                    <div class="w-10 h-10 bg-{{ $color }}-50 text-{{ $color }}-600 rounded-xl flex items-center justify-center shrink-0 border border-{{ $color }}-100">
                        {!! \App\Models\Category::getIconHtml($icon, "w-5 h-5") !!}
                    </div>
                    <div>
                        <p class="text-sm font-bold text-gray-800 uppercase tracking-tight">{{ $cat->name }}</p>
                        <p class="text-[10px] text-gray-400 font-medium">ID: #{{ $cat->id }}</p>
                    </div>
                </div>
                
                <button @click="categoryId = '{{ $cat->id }}'; categoryName = '{{ $cat->name }}'; openSheet('edit-category')" class="p-2 text-gray-400 hover:text-blue-600 transition-colors">
    ...
    @push('bottom-sheet')
    <!-- BOTTOM SHEET FORMS (CATEGORIES) -->
    <template x-if="sheetView === 'add-category' || sheetView === 'edit-category'">
        <div class="space-y-6 text-left">
            <h3 class="text-base font-bold text-gray-900" x-text="sheetView === 'add-category' ? 'Tambah Kategori Baru' : 'Ubah Nama Kategori'"></h3>
            <form :action="sheetView === 'add-category' ? '{{ route('categories.store') }}' : '{{ url('/categories-management') }}/' + categoryId" method="POST" class="space-y-6">
                @csrf
