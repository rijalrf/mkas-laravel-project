        ...
        <div class="min-h-screen flex flex-col max-w-md mx-auto bg-gray-50 shadow-sm">
            ...
            <main class="flex-1 overflow-y-auto pb-36 p-4 space-y-5">
                ...
                @if(session('success'))
                    <div x-data="{ show: true }" x-show="show" x-init="setTimeout(() => show = false, 4000)" 
                         class="bg-emerald-50 border border-emerald-100 p-4 rounded-xl flex items-center gap-3 text-emerald-700 shadow-sm">
                ...
                @if(session('error'))
                    <div x-data="{ show: true }" x-show="show" x-init="setTimeout(() => show = false, 4000)" 
                         class="bg-rose-50 border border-rose-100 p-4 rounded-xl flex items-center gap-3 text-rose-700 shadow-sm">
            ...
            <nav class="fixed bottom-0 left-0 right-0 bg-white border-t border-gray-200 z-40 max-w-md mx-auto h-16 flex items-center shrink-0">
                ...
                <div class="flex-1 flex justify-center -mt-10">
                    <button @click="openSheet('menu')" class="w-14 h-14 bg-blue-600 rounded-full flex items-center justify-center text-white shadow-lg shadow-blue-100 border-4 border-gray-50 active:scale-95 transition-all">
                    ...
                    <div class="overflow-y-auto hide-scroll flex-1">
                        ...
                        <template x-if="sheetView === 'menu'">
                            <div class="space-y-6 w-full text-center">
                                <h3 class="text-md font-semibold text-gray-800 px-1">Buat transaksi baru</h3>
                        ...
                        <template x-if="sheetView === 'categories'">
                            <div class="space-y-4">
                                <div class="flex items-center gap-2 px-1">
                                    <button @click="sheetView = 'menu'" class="p-1 text-gray-400"><svg xmlns="http://www.w3.org/2000/svg" class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2.5"><path stroke-linecap="round" stroke-linejoin="round" d="M15.75 19.5L8.25 12l7.5-7.5" /></svg></button>
                                    <h3 class="text-md font-semibold text-gray-800">Pilih kategori</h3>
                                </div>
                                <div class="bg-white rounded-xl border border-gray-100 overflow-hidden divide-y divide-gray-50 max-h-[60vh] overflow-y-auto hide-scroll">
                            ...
                            <div class="space-y-6">
                                ...
                                <form :action="sheetView === 'filter' ? '{{ route('history.index') }}' : '{{ route('deposits.index') }}'" method="GET" class="space-y-5">
                                    <template x-if="sheetView === 'filter'">
                                        <div class="grid grid-cols-2 gap-3">
                                            <div class="space-y-1.5">
                                                <label class="text-sm font-medium text-gray-700 ml-1">Tgl mulai</label>
                                            ...
                                            <div class="space-y-1.5">
                                                <label class="text-sm font-medium text-gray-700 ml-1">Tgl selesai</label>
                                    ...
                                    <template x-if="sheetView === 'filter-deposit'">
                                        <div class="space-y-1.5">
                                            <label class="text-sm font-medium text-gray-700 ml-1">Pilih bulan</label>
                                    ...
                                    <div class="space-y-1.5">
                                        <label class="text-sm font-medium text-gray-700 ml-1">Status</label>
                        ...
                        <template x-if="sheetView === 'approve-tx' || sheetView === 'reject-tx' || sheetView === 'approve-dp' || sheetView === 'reject-dp'">
                            <div class="space-y-6">
                                ...
                                <form :action="sheetView.includes('tx') ? '/admin/transactions/' + categoryId + '/' + (sheetView.includes('approve') ? 'approve' : 'reject') : '/admin/deposits/' + categoryId + '/' + (sheetView.includes('approve') ? 'approve' : 'reject')" method="POST" class="space-y-4">
                                    @csrf
                                    <div class="space-y-1.5">
                                        <label class="text-sm font-medium text-gray-700 ml-1">Catatan admin (Wajib)</label>
